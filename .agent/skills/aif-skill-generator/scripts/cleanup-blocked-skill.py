...
def run_skills_remove(skill: str, root: Path, dry_run: bool = False) -> int:
	...
	if not npx:
		if dry_run:
			print(
				f"would run (in {root}): npx skills remove -s {skill!r} -y "
...
def validate_installed_skill_path(
	installed: Path, root: Path, skill: str
) -> ValidatedInstalledPath:
	...
	if resolved_n == root_n:
		raise RuntimeError(f"refusing to delete {resolved}: equals --root")
...
def safe_remove_installed(validated: ValidatedInstalledPath) -> None:
	...
	if validated.original_is_reparse:
		...
		if validated.original.exists() or validated.original.is_symlink():
			raise RuntimeError(
				f"failed to remove symlink or junction {validated.original}: path still exists"
			)
...
def main() -> int:
	...
	parser.add_argument(
		"--root", default=".", help="Project root containing skills-lock.json (default: cwd)"
	)
	parser.add_argument(
		"--installed-path",
		default=None,
	...
	if rc != 0 and not args.dry_run:
		...
		print(
			f"warning: `npx skills remove` returned exit {rc}; "
	...
	if not args.dry_run and not verify_lock_clean(lock_path, skill):
		print(f"error: '{skill}' is still present in {lock_path} after patch", file=sys.stderr)
		return 1

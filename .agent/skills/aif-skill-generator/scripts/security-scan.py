...
_add(
	r"ignore\s+(all\s+)?(previous|prior|above|earlier|preceding)\s+(instructions|rules|guidelines|prompts|constraints)",
	"CRITICAL",
	"Instruction override: attempts to discard prior instructions",
)

_add(
	r"disregard\s+(all\s+)?(previous|prior|above|earlier|preceding)\s+(instructions|rules|guidelines|prompts|constraints)",
	"CRITICAL",
	"Instruction override: attempts to discard prior instructions",
)

_add(
	r"forget\s+(everything|all|what)\s+(you|that)\s+(were|was|know|have been)",
	"CRITICAL",
	"Instruction override: memory wipe attempt",
)

_add(
	r"(you\s+are\s+now|from\s+now\s+on\s+you\s+are|pretend\s+(to\s+be|you\s+are)|act\s+as\s+if\s+you\s+are|roleplay\s+as)\s+(?!an?\s+(expert|professional|experienced)\s+(skill|agent|developer|engineer))",
	"CRITICAL",
	"Role hijacking: attempts to override agent identity",
)

_add(
	r"new\s+(system\s+)?(instructions|rules|prompt|role)\s*:",
	"CRITICAL",
	"Instruction injection: defines new system-level instructions",
)

_add(
	r"<\s*system\s*>|<\s*/?\s*system-?(message|prompt|instruction)\s*>",
	"CRITICAL",
	"Fake system tag: injects fake system-level XML tags",
)

_add(
	r"SYSTEM\s*:\s*you\s+(are|must|should|will|have)",
	"CRITICAL",
	"Fake system prefix: impersonates system-level messages",
)

_add(
	r"override\s+(all\s+)?(safety|security|restriction|constraint|limitation|guardrail|filter)",
	"CRITICAL",
	"Safety bypass: attempts to disable safety mechanisms",
)

# ── 2. Data Exfiltration ──────────────────────────────────────────────────────

_add(
	r"(curl|wget|fetch|http\.request|urllib|requests\.(get|post))\s+.{0,50}(\.env|credentials|secret|token|password|api.?key|private.?key)",
	"CRITICAL",
	"Data exfiltration: sends sensitive files/variables to external URL",
)

_add(
	r"(curl|wget)\s+(-[a-zA-Z]\s+)*https?://(?!skills\.sh|github\.com|npmjs\.com|registry\.npmjs\.org)",
	"WARNING",
	"External HTTP request: sends data to unknown external URL",
)

_add(
	r"(cat|read|type)\s+.{0,30}(\.env|credentials|\.ssh|\.aws|\.gnupg|id_rsa|\.netrc|\.npmrc)",
	"CRITICAL",
	"Credential access: reads sensitive credential files",
)

_add(
	r"\$\{?(HOME|USER|LOGNAME|HOSTNAME)\}?.{0,20}(\.env|\.ssh|\.aws|\.config|\.gnupg|secret|credential|token)",
	"CRITICAL",
	"Credential path traversal: constructs paths to sensitive files",
)

_add(
	r"(send|post|upload|transmit|exfiltrate|leak)\s+.{0,40}(secret|credential|token|password|key|\.env|private)",
	"CRITICAL",
	"Explicit exfiltration: instructions to send sensitive data",
)

_add(
	r"base64\s+(encode|decode)\s+.{0,30}(secret|credential|token|password|key|\.env)",
	"CRITICAL",
	"Encoded exfiltration: encodes sensitive data for extraction",
)

# ── 3. Stealth / Deception ────────────────────────────────────────────────────

_add(
	r"(do\s+not|don\'?t|never|must\s+not)\s+(tell|inform|show|reveal|display|mention|alert|warn|notify)\s+(the\s+)?(user|human|operator)",
	"CRITICAL",
	"Stealth instruction: hides actions from user",
)

_add(
	r"(secretly|covertly|without\s+(the\s+)?user\s+(knowing|noticing|seeing))",
	"CRITICAL",
	"Stealth modifier: performs actions without user awareness",
)

_add(
	r"(silently|quietly)\s+(execute|run|delete|remove|send|install|modify|write|overwrite|change|access|read|upload|transmit|exfiltrate|download|inject|replace|drop|connect|post)",
	"CRITICAL",
	"Stealth action: performs dangerous action without user awareness",
)

_add(
	r"(hide|conceal|suppress|mask)\s+(the\s+)?(output|result|error|warning|action|change)",
	"CRITICAL",
	"Output suppression: hides results from user",
)

_add(
	r"invisible|hidden\s+instruction|obfuscate",
	"WARNING",
	"Obfuscation language: may hide malicious intent",
)

# ── 4. Destructive Commands ───────────────────────────────────────────────────

_add(
	r"rm\s+-(r|f|rf|fr)\s+(~|/|\$HOME|\$\{HOME\}|\.\.|/etc|/usr|/var|/tmp)",
	"CRITICAL",
	"Destructive command: recursive delete of critical paths",
)

_add(
	r"(mkfs|dd\s+if=|format\s+[cC]:|\>\s*/dev/sd[a-z])",
	"CRITICAL",
	"Destructive command: disk format or overwrite",
)

_add(r"chmod\s+777\s+/", "CRITICAL", "Dangerous permissions: opens entire filesystem")

_add(
	r"(:\(\)\s*\{\s*:\|:\s*&\s*\}\s*;|fork\s*bomb)",
	"CRITICAL",
...
_add(
	...
	"WARNING",
	"Force push to main: destructive git operation",
)

# ── 5. Configuration Tampering ────────────────────────────────────────────────

_add(
	r"(write|modify|edit|change|overwrite|update)\s+.{0,30}(\.claude|\.cursor|\.codex|\.github|\.gemini|\.junie|\.ai(?:/|\b(?!-))|claude\.json|settings\.json|settings\.local\.json|CLAUDE\.md)",
	"CRITICAL",
	"Config tampering: modifies AI agent configuration",
)

_add(
	r"(write|modify|edit|change|overwrite)\s+.{0,30}(\.bashrc|\.zshrc|\.profile|\.bash_profile|crontab|\.gitconfig)",
	"CRITICAL",
	"Shell config tampering: modifies user shell configuration",
)

_add(
	r"(allowed-tools|allowed_tools)\s*:\s*.*(Bash\s*$|Bash\s+[^(])",
	"WARNING",
	"Unrestricted Bash: skill requests unrestricted shell access (no command pattern)",
)

# ── 6. Privilege Escalation ───────────────────────────────────────────────────

_add(r"sudo\s", "WARNING", "Sudo usage: attempts to escalate privileges")

_add(
	r"(pip|npm|brew|apt|yum)\s+install\s+(?!-)",
	"WARNING",
	"Package installation: installs system-level packages",
)

_add(r"(eval|exec)\s*\(", "WARNING", "Dynamic execution: runs dynamically constructed code")
...
_add(
	...
	"CRITICAL",
	"Hex-encoded payload: long hex-escaped sequence may hide instructions",
)

_add(
	r"\\u[0-9a-fA-F]{4}(\\u[0-9a-fA-F]{4}){7,}",
	"CRITICAL",
	"Unicode-encoded payload: long unicode-escaped sequence may hide instructions",
)

# ── 8. Prompt Injection via Dynamic Context ───────────────────────────────────

_add(
	r"(?<![A-Za-z0-9_])!\s*`[^`]*curl\s",
	"CRITICAL",
	"Dynamic context injection: executes curl in dynamic context substitution",
)

_add(
	r"(?<![A-Za-z0-9_])!\s*`[^`]*(rm|wget|nc|ncat|python|node|bash|sh|eval)\s",
	"WARNING",
	"Suspicious dynamic context: executes potentially dangerous command in dynamic context",
)

# ── 9. Social Engineering ─────────────────────────────────────────────────────

_add(
	r"(this\s+is\s+a\s+test|testing\s+mode|debug\s+mode|maintenance\s+mode)\s*.{0,20}(all\s+restrictions|all\s+rules|safety|security)",
	"CRITICAL",
	'Social engineering: uses "test/debug mode" to bypass restrictions',
)

_add(
	r"(authorized|approved|permitted|allowed)\s+by\s+(admin|system|anthropic|openai|the\s+developer)",
	"CRITICAL",
	"False authority: claims authorization from authority to bypass rules",
)

_add(
	r"(urgent|emergency|critical)\s*:?\s*(override|bypass|disable|ignore)",
	"CRITICAL",
	"Urgency manipulation: uses urgency to justify bypassing rules",
)

# ── 10. Self-Declaration of Safety (scanner evasion) ──────────────────────────

_add(
	r"(this\s+skill|this\s+file|these?\s+patterns?)\s+.{0,30}(verified|audited|safe|secure|approved|reviewed|trusted|harmless)",
	"CRITICAL",
	"Self-declaration of safety: skill claims it has been verified/audited",
)

_add(
	r"((scanner|scan|security.?check|security.?scan|finding|alert|flag|threat)s?.{0,40}false\s+positives?|false\s+positives?.{0,40}(scanner|scan|security.?check|security.?scan|finding|alert|flag|threat)s?)",
	"CRITICAL",
	"Scanner evasion: skill claims findings are false positives",
)

_add(
	r"(this\s+is|are|is)\s+expected\s+behavior",
	"CRITICAL",
	"Scanner evasion: skill dismisses findings as expected behavior",
)

_add(r"safe\s+to\s+ignore", "CRITICAL", "Scanner evasion: skill instructs to ignore findings")

_add(
	r"(not\s+a\s+threat|not\s+malicious)",
	"CRITICAL",
	"Scanner evasion: skill claims its patterns are not threats",
)

_add(
	r"known\s+issue.{0,30}(safe|ignore|skip|harmless|false.positive|not.a.threat|bypass|disable)",
	"CRITICAL",
	'Scanner evasion: uses "known issue" to dismiss security findings',
)

_add(
	r"(ignore|skip|disable|bypass)\s+(the\s+)?(security\s+)?(scan|scanner|check|warning|alert|flag)",
	"CRITICAL",
	"Scanner bypass: skill instructs to skip security scanning",
)

_add(
	r"(scanner|security.?check|security.?scan)\s+(will|may|might|can)\s+(flag|detect|report|show|find)\s+.{0,30}(false|expected|normal|okay|fine|harmless)",
	"CRITICAL",
	"Pre-emptive scanner discrediting: skill warns that scanner will flag it and claims this is normal",
)

_add(
	r"(do\s+not|don\'?t|no\s+need\s+to)\s+(run|execute|perform)\s+.{0,15}(scan|check|verif|audit)",
	"CRITICAL",
	"Scan prevention: skill instructs not to run security checks",
)
...
def load_allowlist(filepath: str) -> list:
	...
	if not isinstance(entries, list):
		print(
			f'{RED}ERROR:{NC} Allowlist must be a JSON array or {{"entries": [...]}}',
			file=sys.stderr,
		)
		sys.exit(3)
	...
	for i, entry in enumerate(entries, 1):
		...
		if "file" not in entry or not isinstance(entry["file"], str) or not entry["file"].strip():
			print(
				f"{RED}ERROR:{NC} Allowlist entry #{i} must include non-empty 'file'.",
				file=sys.stderr,
			)
			sys.exit(3)
		...
		if "severity" not in entry or entry["severity"] not in ("CRITICAL", "WARNING"):
			print(
				f"{RED}ERROR:{NC} Allowlist entry #{i} must include 'severity' = CRITICAL|WARNING.",
				file=sys.stderr,
			)
			sys.exit(3)
		...
		has_match = (
			"match" in entry and isinstance(entry["match"], str) and bool(entry["match"].strip())
		)
		if not (has_description or has_match):
			print(
				f"{RED}ERROR:{NC} Allowlist entry #{i} needs non-empty 'description' or 'match'.",
				file=sys.stderr,
			)
			sys.exit(3)
...
def apply_allowlist(report: dict, allowlist_entries: list) -> dict:
	...
	for file_result in report["file_results"]:
		rel_path = (
			os.path.relpath(file_result["file"], report["path"])
			if path_is_dir
			else os.path.basename(file_result["file"])
		)
		...
		file_result["critical_count"] = sum(
			1 for f in filtered_findings if f["severity"] == "CRITICAL"
		)
		file_result["warning_count"] = sum(
			1 for f in filtered_findings if f["severity"] == "WARNING"
		)
...
def print_report(report: dict):
	...
	if report["total_critical"] > 0:
		...
		print(
			f"{RED}This skill should NOT be installed. It may contain prompt injection or malicious instructions.{NC}"
		)
		return 1
...
def main():
	if len(sys.argv) < 2:
		print(
			"Usage: python security-scan.py [--allowlist <file.json>] [--md-only] [--strict] <path-to-skill-or-SKILL.md>"
		)
	...
	if len(args) > 1:
		print(
			f"{RED}ERROR:{NC} Multiple paths provided. Pass exactly one target path.",
			file=sys.stderr,
		)
		sys.exit(3)

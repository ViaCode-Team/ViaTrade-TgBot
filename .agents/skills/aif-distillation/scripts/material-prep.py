...
def has_sensitive_part(path: Path) -> bool:
	return any(is_sensitive_dir_name(part) for part in path.parts[:-1]) or is_sensitive_file_name(
		path.name
	)
...
def validate_explicit_source_path(path: Path, include_sensitive: bool) -> None:
	...
	sensitive_parts = [
		...
		if is_sensitive_dir_name(part)
		and not (index == 1 and path.parts[0] == "/" and part == "private")
	]
...
def cleanup_output(path_value: str) -> int:
	...
	if not has_valid_marker(target):
		raise RuntimeError(
			f"Refusing cleanup because {TOOL_MARKER_FILE} is missing or invalid: {target}"
		)
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		"--chunk-chars", type=int, default=18000, help="Approximate max characters per chunk."
	)
	parser.add_argument(
		"--cleanup", help="Remove an extraction output directory created by this script."
	)
	parser.add_argument(
		"--include-hidden",
		action="store_true",
		help="Include hidden files and directories during folder extraction.",
	)
	parser.add_argument(
		"--include-sensitive",
		action="store_true",
		help="Include credential-like paths. Unsafe; use only for intentional source material.",
	)
	parser.add_argument(
		"--include-symlinks",
		action="store_true",
		help="Include symlinked files that resolve inside the selected folder and pass the same hidden/sensitive filters.",
	)
...
def main() -> int:
	...
	out_dir = (
		...
		if args.out
		else Path(tempfile.mkdtemp(prefix="aif-distillation-"))
	)
